...
echo "cargo +nightly tauri build -r --target "$1" --runner "$1" -Z trim-paths -Z build-std=core,alloc,std,panic_abort --all-features"

cargo +nightly tauri build -r --target "$1" --runner "$1" -Z trim-paths -Z build-std=core,alloc,std,panic_abort --all-features
